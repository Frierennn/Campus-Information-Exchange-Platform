<template>
  <div class="about">
    ...
    <div class="container">
      <el-page-header @back="goBack" content="活动文章发布"> </el-page-header
      ><br />
      <div class="form-box">
        <el-form label-width="160px" :model="form" :rules="rules" ref="form">
          ...
          <el-form-item label="发布分区">
            <el-select v-model="form.region" placeholder="发布分区">
              <el-option label="摊位活动" value="0"></el-option>
              <el-option label="招聘宣讲" value="1"></el-option>
              <el-option label="比赛报名" value="2"></el-option>
              <el-option label="社团招新" value="3"></el-option>
              <el-option label="商家优惠" value="4"></el-option>
            </el-select>
          </el-form-item>
  ...
  methods: {
    ...
    onSubmit(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          axios
            .post("/upload/addd", this.form)
            .then((res) => {
              let r = res.data;
              if (r.affectedRows == 1) {
                this.$message.success("提交成功！");
                this.$router.push("/vipActivity");
              }
            })
            .catch((err) => {
              console.log(err);
