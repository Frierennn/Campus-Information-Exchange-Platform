            ...
            <el-table-column width="900px">
              <template #default="scope">
                ...
                >
                  <el-avatar size="medium" :src="scope.row.img"></el-avatar
                  >&nbsp;&nbsp;
                  <div>{{ scope.row.user_name }}&nbsp;&nbsp;点赞了我的文章</div>
                </router-link>
                <router-link
            ...
            <el-table-column width="900px">
              <template #default="scope">
                ...
                <div style="padding-left: 50px">
                  <router-link
                    :to="{ path: '/notechange/' + scope.row.notesid }"
                  >
                    {{ scope.row.report }}
                  </router-link>
                </div>
                <div style="padding-left: 50px; padding-top: 10px">
                  ...
                  <el-popconfirm
                    confirm-button-text="好的"
                    cancel-button-text="不用了"
                    @confirm="delreport(scope.row.notesid, scope.row.user_no)"
                    icon="el-icon-info"
                    icon-color="red"
                    title="删除该条通知后无法恢复，是否继续？"
                  >
...
export default {
  ...
  methods: {
    ...
    //获取我的通知
    getData3() {
      axios.get("/comment/report").then((res) => {
        //console.log(res.data);
