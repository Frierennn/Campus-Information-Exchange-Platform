  ...
  <div class="about">
    <div style="width: 100%">
      ...
      <el-button type="primary" icon="el-icon-delete" @click="remtableData"
        >重置</el-button
      ><el-button type="primary" icon="el-icon-edit" @click="vipnewsBaseForm"
        >发布新闻</el-button
      >
    </div>
    ...
    <el-table :data="tableData" style="width: 100%">
      ...
      <el-table-column prop="fl" sortable label="类别"> </el-table-column>
      ...
      <el-table-column prop="img" label="轮播图" width="200" sortable>
        <template #default="scope">
          <el-upload
            v-if="scope.row.img"
            class="upload-demo"
            :data="uploadData"
            action="http://localhost:3000/newss/upload"
          ...
          >
            <el-button
              size="small"
              type="primary"
              @click="uploadindex(scope.row.id)"
              >点击上传</el-button
            >
...
export default {
  ...
  methods: {
    vipnewsBaseForm() {
      this.$router.push({ path: "/vipnewsBaseForm" });
    },
    uploadindex(index) {
      this.uploadData.no = index;
    ...
    getData() {
      axios.get("/newss/news").then((response) => {
